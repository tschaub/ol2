  ...
  /********************************************************/
   ...
   /** 
    ...
    moveByPx: function(dx, dy) {
        ...
        if (dx || dy) {
            if (!this.dragging) {
                this.dragAnimationStart(dx, dy);
            } else {
                this.dragAnimationUpdate(dx, dy);
            }
        }
    },

    /**
     * Property: dragAnimationId
     * {Number}  Identifier for current animation loop.
     */
    dragAnimationId: null,

    /**
     * Property: dragAnimationDX
     * {Number}  Cumulative x-axis pixel displacement since the last repaint.
     */
    dragAnimationDX: null,

    /**
     * Property: dragAnimationDY
     * {Number}  Cumulative y-axis pixel displacement since the last repaint.
     */
    dragAnimationDY: null,

    /**
     * Method: dragAnimationStart
     * {Number}  X-axis pixel displacement
     * {Number}  Y-axis pixel displacement
     */
    dragAnimationStart: function(dx, dy) {
        this.dragAnimationStop();
        this.dragAnimationDX = dx;
        this.dragAnimationDY = dy;
        this.dragging = true;
        this.events.triggerEvent("movestart");
        this.dragAnimationId = OpenLayers.Animation.start(
            OpenLayers.Function.bind(this.dragAnimationRepaint, this)
        );
    },

    /**
     * Method: dragAnimationUpdate
     * {Number}  X-axis pixel displacement
     * {Number}  Y-axis pixel displacement
     */
    dragAnimationUpdate: function(dx, dy) {
        this.dragAnimationDX += dx;
        this.dragAnimationDY += dy;
    },

    /**
     * Method: dragAnimationRepaint
     * This method is called with every available animation frame.  This is
     *     the only method that should be doing any repainting during a drag.
     */
    dragAnimationRepaint: function() {
        var dx = this.dragAnimationDX;
        var dy = this.dragAnimationDY;
        this.dragAnimationDX = 0;
        this.dragAnimationDY = 0;
        this.center = null;
        if (dx) {
    ...
    /**
     * Method: dragAnimationStop
     * Stop a running drag animation loop.
     */
    dragAnimationStop: function() {
        OpenLayers.Animation.stop(this.dragAnimationId);
        this.dragging = false;
    },
